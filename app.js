...
const initializeDbAndServer = async () => {
  try {
    ...
    cpnsole.log(`DB Error ${error.message}`)
    process.exit(1)
  }
}
...
app.post('/register/', async (request, response) => {
  ...
  if (userDBDetails !== undefined) {
    response.status(400)
    reaponse.send('User already exists')
  } else {
...
app.get(
  ...
  async (request, response) => {
    ...
    (SELECT COUNT() FORM Like WHERE tweet_id = "${tweetId}") AS likes,
    (SELECT COUNT() FORM reply WHERE tweet_id = "${tweetId}") AS replies,
...
app.get(
  ...
  async (request, response) => {
    const {tweetId} = request.params
    const getRepliedWuery = `SELECT name, reply
  
FROM user INNER JOIN reply ON user.user_id = reply.user_id 
WHERE tweet_id = "${tweetId}"`
    const repliedUsers = await db.all(getRepliedWuery)
...
app.delete('/tweets/:tweetId/', authentication, async (request, response) => {
  const {tweetId} = request.params
  const {userId} = request
  const getTheTweetQuery = `SELECT * FROM tweeet WHERE user_id = "${userId}" AND tweet_id = "${tweetId}";`
